...
class MillingDataLoad(PHMDataset):
    ...
    ----------
    root : str
        Path to the raw data folder (as a string). Should point to a 'mill.mat' or similar file.

    path_df_labels : pathlib, optional
        Path to the dataframe with the labels (as a string). If not provided, the dataframe must be created.
    ...
    download : bool
        If True, the data will be downloaded from the NASA Prognostics Repository.
...
class MillingPrepOne(MillingDataLoad):
    ...
    #     ), "mill.mat does not exist or is not extracted from zip"
    ...
    #             path_df_labels
    #         )  # path to the labels file with tool class
    #         if self.cut_drop_list is not None:
    #             self.df_labels.drop(
